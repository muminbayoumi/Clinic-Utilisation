---
title: "Clinic Utilisation Report"
...
plotPCD2 + labs(
  x = "Clinic",
  y = "Utilization",
  subtitle = paste0("Black shows Booking Rates while red shows end Utilization Rates, \nRed line is ~ 80% Utilisation \nTotal Number of Clinics: ", table(CU$OneVsTwo)[2], "  between Aug '19 - Jan '20"), shape = "Month"
) +
  theme(
    title = element_text(face = "bold"),
    plot.subtitle = element_text(face = "italic"),
...
```
```{r}
DNAData <- CU %>% group_by(M,OneVsTwo) %>% summarise(MedianD=median(DNARate,na.rm = T))


  DNAData %>% ggplot(aes(x=M,y=MedianD,col=OneVsTwo))+
    geom_boxplot(data=CU,aes(y=DNARate,fill=OneVsTwo),alpha=0.2,color="grey70")+
    geom_point(size=4,shape=18)+
    scale_y_continuous(limits=c(0.05,.128),labels= scales::percent)+
    labs(x="Month",y="Median Non-Attendance")
```
...
[Graph-2.4](#graph2.4) & [Graph-2.3](#graph2.3) demonstrates again underutilized peripheral clinics clinics(p-value:`r round(wilcox.test(CU$True.U~CU$PvP)$p.value,3)`) despite having similar Booking Rates. This appears to be more during certain month. Again however due the small sample sizes it is not possible to perform adequate analysis. 
...
This should bring up the booking rate as well as show us a complete true utilizaition rate to work on next month.
