...
```{r echo=F, error=F, fig.align='center', fig.height=6, fig.width=9, message=FALSE, warning=FALSE, dev='png', dpi=400}
...
plotPCD2 <- PCD2 %>%
  ...
  scale_y_continuous(limits = c(0.55, 1.05), labels = scales::percent) +
  new_scale_colour() +
...
### Graph-2.6 Per Site DNA Rates
```{r warning=FALSE,echo=F,message=FALSE,error=F,fig.height=4, fig.width=6,dev='png',dpi=400,fig.align='center'}


totaldna <-  CU %>% ggplot(aes(y=DNARate,x=PvP))+
  geom_boxplot(aes(fill=PvP))+
  geom_jitter(width = 0.3,aes(col=OneVsTwo))+
  scale_y_continuous(labels=scales::percent)
...
totaldna +
  labs(x=NULL,
       y= "DNA Rate",
       subtitle="This graph shows the distribution of clinic non-attendance rates. \nData demonstrated in form of points and boxplots which repesnent \nsummary statistics. \nAtleast 25% of pilgrim clinics are fully utilized. ",
       fill="Pilgrim Vs Peripheral",
       col="One Man Vs Two Man")+
...
Boxplpots as like on the last graph are extremely usefull once understood. The best analogy I could think of is if you have a class of 100 students. Dividing them into groups would be very useful. Boxplots divide the population into 4 groups of equal proportions of the population -  25%  and then plot those against the scores recieved in this case the score equates to DNA rate with the lowest(zero) being the best .  On Graph-2.6 Atleast 25% of the clinics at pilgrim  had zero DNA  rates as shown by the densely packed set of points. When compared the to peripheral clinics where 25% of the population where under the 5% DNA rate. 
